...
// dropdown. Deriving "has pending change" from a value comparison instead
// (adapterSelect !== settings.sync.adapter) is ambiguous — the parent's async
// settings load (deepMerge mutates the shared reactive object in place) can
// flip settings.sync.adapter at any time, including while this modal is open,
// which looks identical to a real user edit and was wrongly blocking the resync.
  ...
  <dialog id="modal-settings" class="modal backdrop:bg-black/10 backdrop:backdrop-blur-sm">
    <div
      ...
      <div class="flex flex-row h-full">
        <!-- sidebar -->
        <div
          class="w-60 shrink-0 flex flex-col gap-1 bg-base-100 border-r border-base-content/15 px-3 py-5 overflow-y-auto">
