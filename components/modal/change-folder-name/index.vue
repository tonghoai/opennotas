...
<template>
  <dialog id="modal-change-folder-name" :key="modalChangeFolderNameKey"
      ...
      <div class="pt-4">
        ...
        <div class="modal-action">
          <form method="dialog">
            ...
            <button class="btn btn-sm mr-2">{{ $t('app.modal_change_folder_name_cancel') }}</button>
            <button class="btn btn-sm btn-primary" :disabled="!isValidate" @click="handleConfirmNewFolderName">{{
              $t('app.modal_change_folder_name_ok') }}</button>
          </form>
        </div>
      </div>
    </div>
