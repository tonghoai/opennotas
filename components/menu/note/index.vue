...
<template>
  <div class="">
    ...
    <ul v-if="!formNotes.deletedAt" class="menu bg-base-100 rounded-box w-44 border border-neutral animate-fade-down animate-duration-100">
      <li class=""
        @click="handleClickPinNote(props.formNotes.isPinned)">
        <a>
          {{ props.formNotes.isPinned ? $t('app.menu_note_unpin') : $t('app.menu_note_pin') }}
    ...
    <ul v-if="formNotes.deletedAt" class="p-1">
      <li class="px-5 py-1.5 hover:bg-base-300 rounded-lg cursor-pointer" @click="handleClickRestoreNote">
